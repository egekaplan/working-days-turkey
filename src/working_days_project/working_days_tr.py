...
def count_working_days(start_date, end_date, formatted_holidays):
    ...
    while current_date <= end_date:
        ...
        if current_date.weekday() < 5 and formatted_current_date not in formatted_holidays:
            working_days += 1
...
for month in range(1, 13):
    ...
    last_day = 31 if month in [1, 3, 5, 7, 8, 10, 12] else 30 if month != 2 else 29 if ThisYear % 4 == 0 and (ThisYear % 100 != 0 or ThisYear % 400 == 0) else 28
